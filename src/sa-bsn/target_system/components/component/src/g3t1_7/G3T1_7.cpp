#include "component/g3t1_7/G3T1_7.hpp"
...
G3T1_7::G3T1_7(int &argc, char **argv, const std::string &name) :
    Sensor(argc, argv, name, "respiration", true, 1, bsn::resource::Battery("resp_batt", 100, 100, 1), false),
    markov(),
    dataGenerator(),
    filter(1),
    sensorConfig(),
...
void G3T1_7::setUp() {
    ...
    { // Get ranges
        ...
        handle.getParam("respiration_LowRisk", s);
        lrs = bsn::utils::split(s, ',');
        handle.getParam("respiration_MidRisk0", s);
        mrs0 = bsn::utils::split(s, ',');
        handle.getParam("respiration_HighRisk0", s);
        hrs0 = bsn::utils::split(s, ',');
        handle.getParam("respiration_MidRisk1", s);
        mrs1 = bsn::utils::split(s, ',');
        handle.getParam("respiration_HighRisk1", s);
...
double G3T1_7::process(const double &m_data) {
...
void G3T1_7::transfer(const double &m_data) {
    double risk;
    ...
    data_pub = handle.advertise<messages::SensorData>("respiration_data", 10);
...
std::string G3T1_7::label(double &risk) {
    std::string ans;
